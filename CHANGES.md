...
**1. Switching subsystems while watching CCTV now works no matter
what.** v1.6.1's fix relied on the top bar stacking above the
full-screen viewer - which works, but only once the browser has loaded
the updated CSS (a tab that was already open before the update keeps
running the old styles until it's refreshed, so it still felt broken).
v1.6.2 fixes it two ways, neither of which depends on stacking order or
a page reload:

- The full-screen viewer's top bar now has its own **"Switch" menu**
  with every subsystem (Study, AirDrop, Lessons, Settings, ...) - tap
  it, pick where you want to go, and the viewer closes itself as the
  navigation happens. This is rendered INSIDE the viewer, so it works
  on any browser, any age of tab, instantly.
- The CSS rule that lifts the top bar above the viewer now also uses
  `body:has(.cctv-fs)` - a pure-CSS selector that applies the moment
  the overlay exists, so even a stale tab that's still running the old
  JavaScript gets the fix on its next full-screen view.

**2. DEX Labs is now installable as an app** - like the YouTube app
prompt you get in Edge/Brave. Opening DEX Labs on `http://localhost`
(where the service-worker requirement is satisfied) makes Edge/Brave
offer "Install DEX Labs": it gets its own window, its own taskbar
entry, and the DL logo, and runs standalone with no browser chrome. The
pieces: a web app manifest (`public/manifest.json`), DL-logo icons in
192/512/180px (`public/icons/`, generated by the committed
`generate-pwa-icons.ps1` - same blue DL mark as the tray icon), an
Apple home-screen icon, and a deliberately conservative service worker
(`public/sw.js`) that never intercepts `/api/*` (live data, CCTV
streams and snapshots must never be cached) and is network-first
everywhere else, so updates are always fetched fresh and the shell even
loads offline. Install works wherever the site is a secure context
(`localhost`, or an HTTPS reverse-proxy); over plain HTTP on the LAN
(phones), the site simply keeps working as before - no service worker,
nothing changes.

**Also:** the Switch menu's document-level click listener is cleaned up
when the viewer closes (no listener leak across open/close cycles).
