...
// v1.3.1 perf fix: cache in memory instead of hitting disk on every
// read - the Clock UI polls its status endpoint once a second while
// open, which used to mean a synchronous file read every second on top
// of everything else the server is doing. This process is the only
// writer, so a plain in-memory cache kept in sync on write() is safe.
let cache = null;
