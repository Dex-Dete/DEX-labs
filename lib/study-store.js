...
async function getStats(year) {
  ...
  const msBySubjectByDay = {}; // date -> { subjectId -> totalMs } (Study + Rec, for dominant-subject coloring)
  ...
  const heatmap = daysInYear(y).map((date) => {
    ...
    if (subjMap) {
      ...
      for (const [sid, sms] of Object.entries(subjMap)) {
        if (sms > maxMs) { maxMs = sms; dominantSubjectId = sid; }
      }
      const subj = subjectsById[dominantSubjectId];
      dominantColor = (subj && subj.color) || null;
    }
    return { date, ms, minutes, level: levelForMinutes(minutes), dominantSubjectId, dominantColor };
  });
