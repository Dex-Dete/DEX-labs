...
- **CCTV switching made bulletproof.** v1.6.1's topbar-above-viewer
  z-index fix only helped once a tab loaded the new CSS; a long-open
  tab kept the old styles and still felt blocked. v1.6.2 adds: (a) a
  "Switch" menu INSIDE the `.cctv-fs` top bar (`renderSwitchMenu()` in
  cctv.js - fetches /api/settings/subsystems, renders chips, navigates
  by hash; route() -> CCTV.cleanup() closes the viewer), and (b) a
  `body:has(.cctv-fs)` selector alongside `body.cctv-fs-open` in
  cctv.css so the top bar lifts even with stale JS. The document-level
  click listener for the menu is removed in closeFullscreen via
  `overlay._closeMenu`.
- **PWA installability.** New files: `public/manifest.json` (standalone,
  DL icons), `public/icons/icon-{192,512}.png` + `apple-touch-icon.png`
  (generated by committed `generate-pwa-icons.ps1` - System.Drawing,
  blue #2F6690 rounded square + white DL, same as tray icon),
  `public/sw.js` (network-first, NEVER intercepts `/api/*` - CCTV MJPEG
  streams must never go through a promise/cache), registration in
  index.html gated on `window.isSecureContext` (SWs only exist on
  localhost/HTTPS - LAN HTTP clients keep working with no worker).
  Edge/Brave offer "Install" on localhost; phones over LAN HTTP get no
  install prompt (not a secure context) - that's expected, not a bug.

Rules for future work: never add caching/interception for `/api/*` in
sw.js (CCTV streams are infinite responses - `cache.put` would leak);
if new full-screen-ish overlays appear, follow the cctv-fs pattern
(inset viewer + in-viewer escape menu + `:has()`-safe CSS); keep the
PWA icons regenerable by running `generate-pwa-icons.ps1`.
