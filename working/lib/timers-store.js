...
// v1.3.1 perf fix: this used to do a synchronous disk read (and, from
// tick() below, a synchronous disk WRITE) unconditionally every single
// second forever, even when there were zero timers - on slow storage
// (eMMC/older HDD, the kind paired with a low-end Celeron box) that's a
// blocking sync I/O hit on Node's single thread once a second, all day,
// competing with every other request the app is trying to serve at the
// same time. Fixed by keeping the parsed data cached in memory (this
// process is the only writer - nothing else touches timers.json) and
// only touching disk when a write() actually happens.
let cache = null;
...
// Sentinel a mutator can return to signal "nothing actually changed" -
// update() then skips the disk write entirely. Used by tick() so a
// normal "still counting down" second doesn't hit disk at all.
...
function update(mutator) {
  writeQueue = writeQueue.then(() => {
    ...
    if (result !== SKIP_WRITE) write(data);
    return result === SKIP_WRITE ? undefined : result;
  });
  return writeQueue;
}
...
async function tick() {
  // Fast path: nothing to do at all (by far the most common case - most
  // people aren't running a timer/alarm most of the time). Reading from
  // `cache` here costs nothing once it's warm - no disk hit.
  const peek = read();
  if (peek.timers.length === 0) return;

  const now = Date.now();
  let shouldBeep = false;
  let changed = false;

  // Only actually queue a write if something in the data changed this
  // tick - a plain "still counting down, nothing expired" second no
  // longer touches disk at all.
  await update((data) => {
