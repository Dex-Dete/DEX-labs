(() => {
  ...
  let recChipView = null;
  function updateRecChip() {
    if (!recChip || !recChipView || activeSubsystemId === 'study') {
      if (recChip) recChip.hidden = true;
      return;
    }
    const v = recChipView;
    const now = Date.now();
    const el = v.elapsedMs + (v.running ? (now - v.fetchedAt) : 0);
    recChip.textContent = `🎥 ${v.subjectName} · ${fmtMsClock(el)}`;
    recChip.title = `Rec: ${v.subjectName} - ${el} elapsed - tap to open Rec`;
    recChip.hidden = false;
  }
  async function refreshRecChip() {
    try {
      const res = await fetch('/api/study/rec/active');
      const d = await res.json();
      recChipView = d ? Object.assign({}, d, { fetchedAt: Date.now() }) : null;
    } catch (e) { /* offline-tolerant */ }
    updateRecChip();
  }
  if (recChip) {
    recChip.addEventListener('click', () => { location.hash = '#/study/rec'; });
    setInterval(refreshRecChip, 5000);
    setInterval(updateRecChip, 1000);
    window.addEventListener('hashchange', refreshRecChip);
    refreshRecChip(); // show it right away on page load, no 5s wait
  }

  const paperChip = document.getElementById('paper-chip');
  let paperChipView = null;
  function updatePaperChip() {
    if (!paperChip || !paperChipView || activeSubsystemId === 'study') {
      if (paperChip) paperChip.hidden = true;
      return;
    }
    const v = paperChipView;
    const now = Date.now();
    const el = v.elapsedMs + (v.running ? (now - v.fetchedAt) : 0);
    paperChip.textContent = `📝 ${v.subjectName} · ${fmtMsClock(el)}`;
    paperChip.title = `Paper: ${v.subjectName} - ${el} elapsed - tap to open Paper`;
    paperChip.hidden = false;
  }
  async function refreshPaperChip() {
    try {
      const res = await fetch('/api/study/paper/active');
      const d = await res.json();
      paperChipView = d ? Object.assign({}, d, { fetchedAt: Date.now() }) : null;
    } catch (e) { /* offline-tolerant */ }
    updatePaperChip();
  }
  if (paperChip) {
    paperChip.addEventListener('click', () => { location.hash = '#/study/paper'; });
    setInterval(refreshPaperChip, 5000);
    setInterval(updatePaperChip, 1000);
    window.addEventListener('hashchange', refreshPaperChip);
    refreshPaperChip(); // show it right away on page load, no 5s wait
  }
